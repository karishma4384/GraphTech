...
/************************************************************************************************
 * * Suitelet **
 *  * To view the Band Detail Report **
...
define(['N/log', 'N/record', 'N/task', 'N/ui/serverWidget', 'N/util', 'N/url','./../Library/JJ CM NS Utility.js','./../Model/JJ CM Resolver Model GTIL-39.js','./../Templates/JJ CM Resolver Template GTIL-39.js'],
    ...
    (log, record, task, serverWidget, util, url,jjUtil, modelResolver,templateResolver) => {
        ...
        //const DEFAULT_TYPEOFREPORT = "ALL";
        ...
        const form = {
            ...
            setBodyFields() {
                ...
                form.setformFieldProperty(form.bodyFields.custpage_filter_report, {
                    ...
                    isMandatory: true,
                    defaultValue: exports.parameters?.custpage_filter_report || "Brand Detail Report"
                });
                ...
                form.setformFieldProperty(form.bodyFields.custpage_filter_brand, {
                    ...
                    defaultValue: exports.parameters?.custpage_filter_brand || 'ÁLL',
                });
                ...
                form.setformFieldProperty(form.bodyFields.custpage_filter_customer, {
                    ...
                    defaultValue: exports.parameters?.custpage_filter_customer
                });
                //Type of Report
                ...
                form.setformFieldProperty(form.bodyFields.custpage_detail_report, {
                    ...
                    isMandatory: false,
                    defaultValue: exports.parameters?.custpage_filter_report_formatted || "Brand Detail Report"
                });
            ...
            setSublistFields() {
                form.subLists.custpage_sublist_report = form.formObj.addSublist({
                    ...
                    label: "Brand Detail Report"
                });
                const columnMap = {
                    "brand": {
                        "id": "custpage_month", "label": "Current Month", "type": serverWidget.FieldType.TEXT
                    },
                    "Channel": {
                        "id": "custpage_channel", "label": "Customer Type", "type": serverWidget.FieldType.TEXT
                    },
                    "currentYear": {
                ...
                let savedSearchConfigParam = {
                    filterConfig: {
                        report:     exports.parameters?.custpage_filter_report ? exports.parameters?.custpage_filter_report : "Brand Detail Report",
                ...
                const modelSavedSearchResult = fetchModel.models.fetchDetail(savedSearchConfigParam);
                ...
                let classifyResults = fetchModel.models.classifyResults(modelSavedSearchResult.lines, {
                    groupBy: 'brand',
                    groupByFallbackValue: '-NONE-',
                    sumOf: ['currentYear'],
                });
                for (let index = 0, len = classifyResults.length; index < len; index++){
                    for (let key in columnMap){
                        form.subLists.custpage_sublist_report.setSublistValue({
                            id: columnMap[key].id,
                            line: index,
                            value: index == len ?
                                (jjUtil.assignDefaultValue(classifyResults[index][key]?.text, classifyResults[index][key]?.value) || '---') :
                                (jjUtil.assignDefaultValue(classifyResults[index][key]?.text, classifyResults[index][key]?.value) || ' ')
                        });
                    }
                }
                ...
                //     "totalSales": {"value": "0", "text": "100%"},
                //     "previousYear": {"value": "0", "text": null},
                //     "percentage_preyear": {"value": "0", "text": "100%"},
                //     "change_OverYear": {"value": "0", "text": null},
                // });
                // modelSavedSearchResult.lines.push({
                //     "brand": {"value": "SUMMARY", "text": null},
                //     "Channel": {"value": "", "text": null},
                //     ...summaryLine
                // });
            ...
            generateForm(){
                ...
                form.init(serverWidget.createForm({
                    title: "Brand Detail Report"
                }));

                //Associate Client Script with Suitelet
                form.formObj.clientScriptModulePath = "./JJ CS Brand Detail Report GTIL-39.js";
        ...
        const exports = {
            ...
            pdfDownloaded() {
                ...
                log.debug('modelSavedSearchResult', modelSavedSearchResult);
                templateFetch = templateResolver.templateFetch('BRAND DETAIL REPORT PDF');
            ...
            toBeCsvDownloaded() {
                ...
                const modelSavedSearchResult = fetchModel.models.runSearch(savedSearchConfigParam);
                log.debug('modelSavedSearchResult', modelSavedSearchResult);
                templateFetch = templateResolver.templateFetch('BRAND_DETAIL_CSV_REPORT');
                let renderObj = templateFetch.templateRender.init();
            ...
            onRequest(scriptContext){
                try{
                    ...
                    if(this.method == "GET") {
                        ...
                        return exports.context.response.write("SOMETHING_WENT_WRONG_BRAND_DETAIL_REPORT"), false;
                    }
                    return exports.context.response.write("SOMETHING_WENT_WRONG_BRAND_DETAIL_REPORT"), false;
                }catch (err) {
