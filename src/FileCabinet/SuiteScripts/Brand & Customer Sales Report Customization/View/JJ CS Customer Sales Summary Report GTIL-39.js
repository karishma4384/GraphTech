    ...
    function (currentRecord, message,url){
        ...
        function triggerSearch_js39(defaultParam, doNotNavigate) {
            var paramObj = {
                ...
                custpage_filter_customer: recordObj.getValue({fieldId: 'custpage_filter_customer'}),
            ...
            for (var key in paramObj) {
                if((!paramObj[key]) && (paramObj["custpage_filter_customer"])){
            ...
            paramObj.custpage_filter_customer_formatted = recordObj.getText({fieldId: 'custpage_filter_customer'})
        ...
        function triggerExport_pdf() {
            ...
            if (pageLength == 0)
                ...
                message.create({
                    title: 'REPORT EXPORT',
                    message: 'Please wait and do not navigate while we prepare the PDF',
        ...
        function triggerCsvExport_gtil39() {
            var paramObj = {
                ...
                custpage_filter_customer: recordObj.getValue({fieldId: 'custpage_filter_customer'}),
                custpage_filter_invoice: recordObj.getValue({fieldId: 'custpage_filter_invoice'}),
                custpage_filter_credit: recordObj.getValue({fieldId: 'custpage_filter_credit'}),
                custpage_filter_pageindex: recordObj.getValue({fieldId: 'custpage_filter_pageindex'})
                ...
                for (var index = 1; index <= pageLength; index++) {
                    (function (iNum, uniqueUrl) {
                        ...
                        fetch(uniqueUrl, {
                            ...
                            var a = document.createElement('a');
                            a.href = url;
                            a.download =iNum + "-" + (Date.now() / 1000) + ".csv";
                            document.body.appendChild(a); // we need to append the element to the dom -> otherwise it will not work in firefox
                            a.click();
                            a.remove();  //afterwards we remove the element again
                        });
